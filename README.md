...
Astapor's negotiated exchange remains pending. The Maps tab embeds Jonathan Roberts's existing **The Known World** geographic map from *The Lands of Ice and Fire*, © George R. R. Martin, 2012. It loads the published preview from Fantastic Maps via its image CDN, so an internet connection is needed. Zoom and scrolling help explore the labels, although detail is limited by the source preview's resolution. There are no character paths or story overlays. Source: [the artist's map article](https://www.fantasticmaps.com/2013/03/official-world-map-of-a-song-of-ice-and-fire/).
...
- `maps.js`: embedded map loading feedback and zoom controls.
